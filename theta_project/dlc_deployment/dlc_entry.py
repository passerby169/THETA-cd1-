...
# 标记 DLC 环境，确保 THETA 内部配置按容器挂载路径解析。
...
def parse_training_metrics(stdout: str, total_epochs: int) -> list[dict]:
    """
    从 run_pipeline.py 的 stdout 中解析每轮训练的 loss 值。

    匹配的典型格式（来自 THETA 的 ETM trainer）：
        Epoch [1/50] - Loss: 1.234
        epoch: 1, loss: 0.987
        "epoch": 2, "loss": 0.876
    ...
    -------
    list[dict]
        [{"epoch": 1, "loss": 1.234, "timestamp": "..."}, ...]
    """
    import re
    metrics = []
    seen_epochs = set()
    patterns = [
        re.compile(r"(?i)Epoch\s*\[?\s*(\d+)\s*/\s*\d+\]?\s*[-–]?\s*Loss[:\s]+([0-9.]+)"),
        re.compile(r"(?i)epoch[:\s]+(\d+)[^0-9]+loss[:\s]+([0-9.]+)"),
        re.compile(r"(?i)\"epoch\"\s*:\s*(\d+)[^}]+\"loss\"\s*:\s*([0-9.]+)"),
        re.compile(r"(?i)step\s+(\d+).*loss\s*([0-9.]+)"),
    ]
    for line in stdout.splitlines():
        for pat in patterns:
            ...
            if m:
                ...
                if epoch not in seen_epochs and epoch <= total_epochs:
...
def run_training(config: dict) -> list[dict]:
    ...
    -------
    list[dict]
        每轮的 {"epoch": int, "loss": float, "timestamp": str} 列表，用于写入 training_log.json
    """
    ...
    return parse_training_metrics(result.stdout, config["epochs"])
...
def save_training_log(config: dict, status: str, metrics: list[dict] | None = None):
    """
    ...
    ----------
    config : dict
        DLC 配置字典（包含 run_id、job_id 等）
    status : str
        "succeeded" | "failed"
    metrics : list[dict] | None
        每轮训练指标，由 run_training() 从 stdout 解析得到。
        格式: [{"epoch": 1, "loss": 1.234, "timestamp": "..."}, ...]
        前端 /api/train/{job_id}/metrics 依赖此字段渲染训练曲线。
    """
    ...
    print(f"[INFO] Training log saved: {log_file}  ({len(metrics or [])} epoch entries)")


def notify_server(config: dict, status: str):
    """通知服务器训练完成，回传 job_id 和 run_id 以便后端拼接 OSS 结果路径"""
    ...
    try:
        payload_job_id = int(job_id) if job_id and job_id.strip() else 0
    except ValueError:
        payload_job_id = 0
    payload = {
...
def main():
    ...
    try:
        ...
        # 5. 模型训练 — 捕获每轮 metrics 用于前端训练曲线
        training_metrics = run_training(config)
    ...
    # 7. 保存训练日志（含 metrics 数组，前端通过 /api/train/{job_id}/metrics 读取）
    save_training_log(config, training_status, metrics=training_metrics)
